...
def test_checkpoint_round_trip(tmp_path):
    ...
    assert restored.in_channels == NUM_FEATURES
    assert restored.num_classes == 2


def test_checkpoint_restores_architecture_flags(tmp_path):
    """A no-PBR/no-grid-context model must come back in the same shape."""
    model = build_model(in_channels=NUM_FEATURES, use_pbr=False, use_grid_context=False)
    path = str(tmp_path / "ablated.pt")
    save_checkpoint(model, path)

    restored = load_checkpoint(path)
    assert restored.pbr is None
    assert restored.use_grid_context is False
...
def test_checkpoint_stores_feature_contract(tmp_path):
    ...
    assert payload["in_channels"] == NUM_FEATURES
